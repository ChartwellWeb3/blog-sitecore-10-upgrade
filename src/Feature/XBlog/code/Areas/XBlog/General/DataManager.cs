...
namespace Sitecore.Feature.XBlog.Areas.XBlog.General
{
    public class DataManager
    {
        ...
        public static string GetBlogName(Item item)
        {
            BlogHome homeItem = GetBlogHomeItem(item).CreateAs<BlogHome>();
           
                return homeItem.Name;
        }
        #endregion
        ...
        public static string GetUrlValByPattern(string url, string urlPattern)
        {
            ...
            string decodedItemName = "";
            if (splitUrl.Length > 1)
             decodedItemName = MainUtil.DecodeName(splitUrl[1]);
            else
                decodedItemName = MainUtil.DecodeName(splitUrl[0]);
            try
            {
                char[] splitChars = { '=' };
                ...
                string[] splitUrl1 = decodedItemName.Split(splitChars);
                if(splitUrl.Length > 1)
                { return WebUtility.UrlDecode(splitUrl[1]); }
            }
            catch (Exception ex)
            {
